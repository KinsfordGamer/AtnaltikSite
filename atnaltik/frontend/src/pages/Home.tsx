...
const Home: React.FC = () => {
  ...
  const [stats, setStats] = useState<any>({ animes: 0, episodes: 0, users: 0, seasons: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const [animesResp, statsResp] = await Promise.all([
          AnimeAPI.getAnimes(),
          AnimeAPI.getStats()
        ]);
        setAnimes(animesResp.data);
  ...
  return (
    <div className="pb-20 overflow-hidden">
      ...
      <section className="relative h-[90vh] w-full flex items-center px-6 md:px-20">
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-6xl md:text-8xl font-black italic mb-6 leading-[0.9] tracking-tighter text-gradient break-words">
      ...
      {/* Info Section */}
      <section className="mt-20 md:mt-40 px-6 md:px-20">
        <div className="glass-card p-8 md:p-20 relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
          <div className="absolute top-0 right-0 w-96 h-96 bg-primary/10 rounded-full blur-3xl -mr-48 -mt-48" />
          <div className="flex-grow">
            <h2 className="text-3xl sm:text-5xl font-black italic mb-6 leading-tight">
            ...
            <div className="flex flex-wrap gap-6 md:gap-10">
              <div>
                <div className="text-3xl md:text-4xl font-black text-white mb-1">{stats.animes || 0}</div>
                <div className="text-gray-500 text-xs md:text-sm font-bold uppercase tracking-widest">Animelar</div>
              </div>
              <div>
                <div className="text-3xl md:text-4xl font-black text-white mb-1">{stats.episodes || 0}</div>
                <div className="text-gray-500 text-xs md:text-sm font-bold uppercase tracking-widest">Qismlar</div>
              </div>
              <div>
                <div className="text-3xl md:text-4xl font-black text-white mb-1">{stats.users || 0}</div>
                <div className="text-gray-500 text-xs md:text-sm font-bold uppercase tracking-widest">Foydalanuvchilar</div>
              </div>
            </div>
          </div>
